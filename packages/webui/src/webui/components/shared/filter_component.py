...
class FilterComponent:
    @classmethod
    def render(cls, session_key_prefix: str, filters: List[Filter], expanded: bool = False) -> Dict[str, Any]:
        ...
        with st.expander("Filters", expanded=expanded):
            with st.form(f"{session_key_prefix}_filters_form_{st.session_state[form_key]}"):
                ...
                for col, current_filter in zip(st.columns(len(filters)), filters):
                    with col:
